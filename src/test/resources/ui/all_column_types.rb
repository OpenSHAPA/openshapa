...
begin
  ...
  if !$dataStore.pred_name_in_use("test0")
    ...
    pve0.append_formal_arg(farg)
    farg = PredFormalArg.new($dataStore, "<pred>")
    pve0.append_formal_arg(farg)
    farg = QuoteStringFormalArg.new($dataStore, "<qstring>")
    pve0.append_formal_arg(farg)
    farg = UnTypedFormalArg.new($dataStore, "<untyped>")
    pve0.append_formal_arg(farg)
...
#      offset = onset + (dd * 200)
      ...
      if coltypes[cc] == MatrixVocabElement::MatrixType::FLOAT
        ...
        fdv3 = PredDataValue.new($dataStore, fargid, Predicate.new($dataStore, predID0))
        fargid = pve0.get_formal_arg(4).get_id()
        fdv4 = QuoteStringDataValue.new($dataStore, fargid, "quote_string")
        fargid = pve0.get_formal_arg(5).get_id()
        fdv5 = UndefinedDataValue.new($dataStore, fargid)

        if dd == 0
          # construct a predicate with null args in first cell of column
          pred = Predicate.new($dataStore, predID0)
        else
          pred = Predicate.new(Predicate.construct($dataStore, predID0, fdv0, fdv1, fdv2, fdv3, fdv4, fdv5))
        end
        ...
        fdv4 = UndefinedDataValue.new($dataStore, fargid)

        if dd == 0
          # construct a matrix with null args in first cell of column
          mat = Matrix.new($dataStore, matID0)
        else
          mat = Matrix.new(Matrix.construct($dataStore, matID0, fdv0, fdv1, fdv2, fdv3, fdv4))
        end
      end
